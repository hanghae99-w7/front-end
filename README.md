...
### 1.정렬하기 기능 구현중 정렬하기를 눌렀을때 리덕스를 이용하여 빈 배열로 만든 후에 정렬에 맞게 조회를 하는 로직을 구현했는데 원래 있던 상품 카드들이 삭제되는 현상이 발생했다.<br>
...
수 있는 유틸폴더에 만들어주기 <br><br> ✏️ 스타일 : 반응형에서 height 값은 고정시
키지 않기. 넓이는 유동적으로 늘었다 줄었다 하지만 height 값은 데스크탑이나 아이
패드, 모바일 등등 고정되어있는 경우가 많음 height 대신 line-height로 감싸서 안에
값을 퍼센트로 지정 하고 겉에 하이트를 따로 지정 해주기<br><br> ✏️ 폰트 패밀리 :
